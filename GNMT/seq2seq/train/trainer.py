...
from seq2seq.train.fp_optimizers import Sketch_Embed_Optimizer
...
from seq2seq.timer import Timer
...
class Seq2SeqTrainer:
    ...
    def __init__(self,
        ...
        self.timer = Timer(log_fn=self.metric)
        ...
        if distributed:
            if cuda:
                ...
                self.criterion = self.criterion.cuda(self.device)
        ...
        if math == 'fp16':
            ...
            self.fp_optimizer = Fp32Optimizer(self.model, grad_clip, self.local_rank, self.timer)
            params = self.model.parameters()
        elif math == 'sketch_embed':
            self.fp_optimizer = Sketch_Embed_Optimizer(self.model, grad_clip, self.local_rank, self.timer)
            params = self.model.parameters()
    ...
    def iterate(self, src, tgt, update=True, training=True):
        ...
        if self.cuda:
            ...
            tgt = tgt.cuda()
        with self.timer("batch.forward", float(self.epoch)):
            if self.batch_first:
            ...
            # loss = loss + 1e-5 * l1_regularization
    ...
    def feed_data(self, data_loader, training=True):
        ...
        if training:
            logging.info(f'epoch: {self.epoch} ID: {torch.distributed.get_rank()}\n {self.timer.summary()}')
            if torch.distributed.get_rank() == 0:
                self.timer.save_summary(os.path.join(os.path.dirname(__file__), "timer_summary.json"))
    ...
    def save(self, identifier=None, is_best=False, save_all=False):
        ...
        if self.distributed:
            ...
            # we use customize distributed, so the model does not have 'module'

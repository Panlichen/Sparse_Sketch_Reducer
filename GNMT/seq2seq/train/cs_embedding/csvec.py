...
class CSVec(object):
    ...
    def __init__(self, d, c, r, doInitialize=True, device=None,
        ...
        Args:
            d: the shape of the sparse embedding gradient
        ...
        self.d = int(d[0]*d[1]) # vector dimensionality
        ...
        h2 = hashes[:,1:2]
        h1[:] = 1
        # self.buckets = ((((h1 + tokens) * h2) * (tokens + h3) * (tokens + h4) ))% LARGEPRIME % self.c
        self.buckets = ((h1 * tokens) ) % LARGEPRIME % self.c
    ...
    def accumulateVec(self, vec):
        ...
        """
        vec_indices = vec.indices()[0]*vec.size()[1]+vec.indices()[1]
        # print(vec_indices)
        assert(len(vec.values().size()) == 1 and vec_indices.size()[0] == vec._nnz())
        assert(vec_indices.dtype == torch.long)
        self.bitmap[:]=0

        self.bitmap[vec.indices()[0].unique_consecutive()] = 1
        # the vector is sketched to each row independently
        for r in range(self.r):
            buckets = self.buckets[r,vec_indices].to(self.device)
            signs = self.signs[r,vec_indices].to(self.device)
            # print(buckets, signs)
            ...
            for blockId in range(self.numBlocks):
                ...
                self.table[r,:] += torch.bincount(
                                    input=offsetBuckets,
                                    weights=offsetSigns * vec.values()[start:end],
                                    minlength=self.c
                                   )
    ...
    def _findAllValues(self, table, vec_indices):
        if self.numBlocks == 1:
            vals = torch.zeros(self.r, vec_indices.size()[0], device=self.device)
            for r in range(self.r):
                buckets = self.buckets[r,vec_indices].to(self.device)
                signs = self.signs[r,vec_indices].to(self.device)
                vals[r] = (table[r, buckets]
                           * signs)
            ...
            for blockId in range(self.numBlocks):
                ...
                for r in range(self.r):
                    ...
                    vals[r] = (self.table[r, offsetBuckets]
                                * offsetSigns)
                medians[start:end] = vals.mean(dim=0)
            return medians

    def decompress(self, table, bitmap):
        assert(len(bitmap.size()) == 1)
        # actually bitmap is in self.device and int64, so the indices_row is in self.device
        indices_row = torch.nonzero(bitmap).expand((-1, self.embedding_size[1])).reshape(-1).cuda(self.device)
        # indices_col is torch.int64
        indices_col = torch.arange(self.embedding_size[1], device=self.device).expand((bitmap.count_nonzero(), -1)).reshape(-1)
        vec_indices = indices_row * self.embedding_size[1] + indices_col
        embedding_values = self._findAllValues(table, vec_indices)
        assert(embedding_values.size() == indices_col.size())
        embedding_indices = torch.stack([indices_row, indices_col], dim=0)
        embedding_sparse_gradient = torch.sparse_coo_tensor(embedding_indices, embedding_values, self.embedding_size, device=self.device)
        return embedding_sparse_gradient

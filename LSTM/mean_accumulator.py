...
class MeanAccumulator:
    ...
    def reduce(self):
        ...
        if isinstance(self.average, dict):
            ...
            device = "cuda:"+str(torch.distributed.get_rank()%4) if torch.distributed.get_backend() == "nccl" else "cpu"
    ...
    def _update(self, value, weight):
        ...
        if isinstance(self.average, torch.Tensor):
            # print('average: ', self.average, 1.0-alpha)
            self.average = self.average * (1.0 - alpha)
